...
int send_frame(int parameter, int seq_number, int ack_num)
{
 //printf("sending a frame\n");
  ...
  //sprintf (payload, "ABCDEFGHIJKLMNOPQRSTUVWXYZ");
  alpha9(parameter);
  //printf("%s\n",a);
  strcpy(payload,a);
 ...
 // printf ("Index for interface %s is %i\n", interface, device.sll_ifindex);
  ...
  tcphdr.th_seq = htonl (seq_number);

  // Acknowledgement number (32 bits)
  tcphdr.th_ack = htonl (ack_num);
